...
lac_sez %<>% mutate_at(vars(first_y), as.integer)
```
...
The next step consists in adding data that is relevant to assess the kind of tax-benefit available. 
...
Finally, tax rates for jurisdictions available in neither database were searched in the IBFD Country Tax Guides.
...
lac_sez %<>% filter(sez_or_disad_area!="disad_area")
...
lac_sez %<>% mutate(category = case_when(sez_no==0~cl[8], cdt_rate==0~cl[7], is.na(sez_rate_cdt)~cl[5], holiday_time<11~cl[2],sez_rate_cdt==0|holiday_time>10~cl[1], sez_perc_cdt==1~cl[6], sez_perc_cdt<0.5~cl[3], sez_perc_cdt>=0.5~cl[4])) 
...
lac_sez_rec <- lac_sez %>% group_by(iso3c, regime_id) %>% slice_max(year) %>% ungroup
...
lac_sez_rec %>% select(category,country.x) %>% group_by(category) %>% group_nest() %>% kable() 
...
ggplot(dat) +
 geom_sf_pattern(aes(fill=sez_perc_cdt, pattern_alpha=holiday_dummy), pattern="crosshatch", pattern_fill="white", pattern_spacing=0.03)+
  ...
  scale_fill_gradient(low = "#FFFFFF", high="#000000", guide=guide_colorbar(keywidth=0.7, keyheight=0.7))+
  scale_pattern_alpha_discrete(range = c(0,1), guide=guide_legend(keywidth=0.7, keyheight=0.7))+
  theme_minimal()+
...
ggarrange(ggarrange(latam, car, ncol=2, nrow=1), leg, ncol=1, nrow=2)
