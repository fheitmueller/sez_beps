...
regimes_sez_harmful <- regimes_sez %>% filter(assessment=="harmful")
regimes_sez_not_harmful <- regimes_sez %>% filter(assessment!="harmful")
regimes_sez_harmful %<>% pivot_wider(id_cols=c("iso3c", "reaction"), names_from ="institution", values_from="regime_name")
regimes_sez_not_harmful %<>% pivot_wider(id_cols=c("iso3c", "reaction"), names_from ="institution", values_from="regime_name")

regimes_sez_harmful   %<>% filter(!(iso3c=="ABW"&OECD_FHTP=="Free zone"))

regimes_sez <- bind_rows(regimes_sez_harmful, regimes_sez_not_harmful, .id="harmful_SEZ_regimes")
regimes_sez %<>% mutate(harmful_SEZ_regimes = recode(harmful_SEZ_regimes, "1" = "Yes", "2"="No"))

regimes_sez  %<>% mutate(Country = countrycode(iso3c, "iso3c", "country.name"))
regimes_sez %<>% mutate_at(.vars=c(4,5), str_replace_all, "NULL", "")
regimes_sez %<>% relocate(Country, .before=harmful_SEZ_regimes)
regimes_sez %<>% relocate(reaction, .after=COCG)
regimes_sez %<>% relocate(OECD_FHTP, .after=COCG)
regimes_sez %<>% select(-iso3c)
regimes_sez %<>% group_by(harmful_SEZ_regimes) %>% arrange(Country, .by_group = TRUE)

regimes_sez %>% kable %>% kable_styling()
```
